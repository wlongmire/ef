...
class Base<?php echo ucfirst($this->getModuleName()) ?>GeneratorHelper extends sfModelGeneratorHelper
{
  ...
  {
    return '<li class="a-admin-action-edit">'.link_to('<span class="icon"></span>'.__($params['label'], array(), 'apostrophe'), $this->getUrlForAction('edit'), $object, array('class'=>'a-btn icon a-edit' . ($params['label'] ? '' : no-label))).'</li>';
  }
  
  public function linkToNewChild($object, $params)
  {
    return '<li class="a-admin-action-new-child">'.link_to('<span class="icon"></span>'.__($params['label'], array(), 'apostrophe'), $this->getUrlForAction('object'), $object->toParams() + array('action' => 'newChild'), array('class'=>'a-btn icon a-add' . ($params['label'] ? '' : no-label))).'</li>';
  }
