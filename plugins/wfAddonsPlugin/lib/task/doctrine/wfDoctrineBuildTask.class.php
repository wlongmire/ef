...
/**
 * Our version of wfDoctrineBuildTask so we can call our own versions of BuildFilters and BuildForms.
 *
 * It is SO DUMB that we have to do all of this just to change one option in a task.
 * 
 */
 ...
 */
class wfDoctrineBuildTask extends sfDoctrineBaseTask
{
  const
    BUILD_MODEL   = 1,
    BUILD_FORMS   = 2,
...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    {
      $task = new wfDoctrineBuildFormsTask($this->dispatcher, $this->formatter); //changed from sfDoctrine
      $task->setCommandApplication($this->commandApplication);
      $task->setConfiguration($this->configuration);
      $ret = $task->run();
    ...
    {
      $task = new wfDoctrineBuildFiltersTask($this->dispatcher, $this->formatter); //changed from sfDoctrine
      $task->setCommandApplication($this->commandApplication);
      $task->setConfiguration($this->configuration);
      $ret = $task->run();
