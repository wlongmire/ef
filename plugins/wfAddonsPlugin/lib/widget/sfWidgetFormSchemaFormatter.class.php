...
/**
 * This is a replacement of sfWidgetFormSchemaFormatter in an attempt to support attributing errors (and eventually, help) messages
 * to the elements that they belong to. It should currently be disabled, check core.php to check.
 */
abstract class sfWidgetFormSchemaFormatter
{
  ...
  protected
    ...
    $errorListFormatInARow     = '  <ul class="error-list" %htmlfor%>%errors%</ul>',
  ...
  public function formatRow($label, $field, $errors = array(), $help = '', $hiddenFields = null, $name = null)
  {
    return strtr($this->getRowFormat(), array(
      '%label%'         => $label,
      '%field%'         => $field,
      '%error%'         => $this->formatErrorsForRow($errors, $name),
      '%help%'          => $this->formatHelp($help, $name),
  ...
  public function formatErrorRow($errors, $name = null)
  {
    ...
    return strtr($this->getErrorRowFormat(), array('%errors%' => $this->formatErrorsForRow($errors, $name)));
  }

  public function formatErrorsForRow($errors, $name = null)
  {
    if (null === $errors || !$errors)
    {
      return '';
    ...
    $pairs['%errors%'] = implode('', $this->unnestErrors($errors));
    $pairs['%htmlfor%'] = $name ?
//        sprintf('htmlfor="%s"', $this->widgetSchema->generateId($this->widgetSchema->generateName($name))) :
        sprintf('htmlfor="%s"', 'test[radio]') :
        '';

    return strtr($this->getErrorListFormatInARow(), $pairs);
  }
