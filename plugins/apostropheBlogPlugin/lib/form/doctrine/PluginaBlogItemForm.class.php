...
abstract class PluginaBlogItemForm extends BaseaBlogItemForm
{
  ...
  public function setup()
  {
    ...
    if($user->hasCredential('admin'))
    {
      ...
      {
        $q = Doctrine::getTable('sfGuardUser')->createQuery('u')->leftJoin('u.Permissions p WITH p.name = "cms_admin"')->leftJoin('u.Groups g')->leftJoin('g.Permissions gp WITH gp.name = "cms_admin"')->addWhere('(u.is_super_admin IS TRUE) OR gp.name = "cms_admin" OR p.name = "cms_admin"');
      }
