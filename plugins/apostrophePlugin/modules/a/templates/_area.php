<?php
  ...
  $displayRandomSlot = isset($options['displayRandomSlot']) && $options['displayRandomSlot'];
?>
<?php use_helper('a') ?>
...
<?php $i = 0; foreach ($slots as $permid => $slot): ?>
  ...
  <?php if ($displayRandomSlot && !$slot->isNew()): ?>
    <?php if (!isset($randomPermid)): ?>
      <?php $randomPermid = array_rand($slots) ?>
    <?php endif ?>
    <?php if ($permid != $randomPermid): ?>
      <?php continue ?>
    <?php endif ?>
  <?php endif ?>
	<?php if ($infinite): ?>
